...
| ------------- | ------------- | --------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `migrations/` | ✅ 41 applied | Schema + RLS migrations (Supabase CLI)              | [data-model](../docs/10-architecture/data-model.md)                                                      |
...
Everything runs inside a transaction that `ROLLBACK`s, so it is safe against a live database and leaves no fixtures behind. Verified green against the live database: `rls_membership.sql` 22/22, `rls_workflow.sql` 33/33, `materialise_plan.sql` 19/19, `artifacts.sql` 12/12, `room_sources.sql` 14/14, `document_supersession.sql` 9/9, `project_membership.sql` 13/13, `marketing_rls.sql` 39/39, `storage_artifacts.sql` 11/11, `materialise_campaign.sql` 41/41.
