...
> Live. `config.toml`, `seed.sql`, and **29 applied migrations** in `migrations/` covering identity, chat, security-definer hardening, table grants, profile co-member visibility, the Realtime presence policy, the RAG schema, hybrid search, action embeds, the default-privilege cleanup, room ownership + the flywheel table, the workflow DAG, the workflow-function hardening, plan materialisation, the scheduler's selection query, artifacts, the question card, run leases, the ticker claim, room sources, the two that let the corpus hold crawled pages (a per-document `source_url`, and the in-force narrowing of `documents_source_hash_idx`), and the one that lets a room hold more than one visible project. Bring up a local stack with `supabase start` (see [DEVELOPMENT.md](../DEVELOPMENT.md)).
>
