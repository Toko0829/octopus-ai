...
`ArtifactEmbedPayload` gains the file ids and their content types, and **never a URL**. A signed download link is a ten-minute bearer credential; the payload is stored in `action_embeds` and re-broadcast on every room update, so a link in it would be a credential written to a table and handed to everyone in the room for as long as the row lives. That half stands.

**The other half of this decision was wrong and is amended.** It said the stream re-renders on every broadcast, so an inline `<img>` would mint one credential per artifact per re-render, and concluded that the card should print a count and send people to the panel. React does not work that way: messages are keyed by id, so a broadcast re-renders the list without re-mounting a card, and an effect keyed on the artifact runs once per mount. The claim was reasoned wrong and then written into this ADR and three module docs, and it survived until somebody generated a real picture and asked why it was not in the chat.

**The card renders the images.** The real cost is smaller than the one that was claimed and is paid for directly: a room with many delivered images would mint many credentials at once, for every viewer, without anybody asking, so each image fetches its link only when its card is actually on screen. `IntersectionObserver`, disconnected on first fire, so it is one fetch per card per session however much the list re-renders. The panel keeps its own arm unchanged.

The argument that decided it is the one this card was built on. An approved step used to write its work into a table only SQL could reach, which read as the product having stopped. A picture somebody has to go and find in a panel is that same defect, one size smaller.
