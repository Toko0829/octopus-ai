...
| `waitpoint-expiry.md`      | Durable run suspended past its deadline              | Inspect the task row and its `events`; re-offer or escalate to ops; never leave a run hanging.                                      |
...
| `stuck-run.md`             | Agent run not progressing                            | Read the task's `events` trail and `task_runs` attempts; check tool errors; resume or cancel with replan diff.                                     |
