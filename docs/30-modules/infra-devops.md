...
>
> One CI property survives from the metered era and is still right:
>
> - **The job is scoped to `services/ai/**` and this workflow.** A README-only PR once spent ~81 rerank calls proving nothing and then failed on the quota it had just exhausted. Scope is computed by one `git diff` in a tiny preceding job rather than a filter action (rule 20), and **falls open** when the base commit cannot be resolved (first push, force-push): skipping on an unknown diff would drop the gate exactly when history is unusual.
>
