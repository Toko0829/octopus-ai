...
> **The Cohere key currently in use is a trial key, capped at 10 calls per minute**, and query decomposition made the eval far denser than that cap: a positive case costs one rerank for the goal plus one per sub-query, so the 15-case set reaches ~81 calls. The job sets `COHERE_RERANK_RPM=8`, which holds it under the cap at roughly **10 minutes of wall clock that is bought entirely by the key, not by the work**. The same cap applies to **production** retrieval, and decomposition made it tighter there too: a single goal can spend seven of the ten calls in a minute. Moving to a production key is a prerequisite for real traffic, not just for the eval.
>
> Two CI properties exist because of that shared quota, and both were learned by failing:
>
> - **The job is scoped to `services/ai/**` and this workflow.** A metered gate must not run where it cannot measure anything. A README-only PR spent ~81 rerank calls proving nothing and then failed on the quota it had just exhausted. Scope is computed by one `git diff` in a tiny preceding job rather than a filter action (rule 20), and **falls open** when the base commit cannot be resolved (first push, force-push): skipping on an unknown diff would drop the gate exactly when history is unusual.
> - **The job holds a repository-wide concurrency group**, not the workflow's ref-scoped one, because the quota belongs to the key rather than the branch. A PR run and its own merge-to-`main` run were separate groups, ran together, and the second was rejected on its first call. `cancel-in-progress` is `false`: queueing costs time, cancelling would discard a half-finished measurement in favour of one competing for the same quota.
>
