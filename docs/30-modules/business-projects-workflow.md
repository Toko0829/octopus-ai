...
The marketplace half (`MATCHING` through `PAID`) had no code behind it until the matcher landed, **which was slice 4** of the sequence in [human-nodes-marketplace.md](human-nodes-marketplace.md). The states existed anyway: the machine was specified here in full, and adding them later would have meant editing it twice and re-deriving arcs already written down. The marketplace **domain tables** landed ahead of it (`20260831120000` … `20260831123000`) with no writer at all, so for a while the states were unreachable and `escalated` was a dead end with twelve tasks sitting in it. Slices 4 to 8 then supplied the walkers, in order: the matcher and the offer cascade, acceptance with escrow, the work loop and reassignment, the payout, and disputes. Every state in that half is reachable now and the sections below record which slice produced each arc.
...
| ------------------------------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `matching → failed`             | **never**   | ❌ booked for slice 4 as "no eligible node is a real outcome", then decided against when slice 4 built the matcher ([ADR-0018](../40-adr/0018-offer-exhaustion-returns-the-step-to-its-owner.md)): `failed` is terminal and would block every dependent, so an empty pool returns the step to `escalated` instead                                                       |
| `offered → failed`              | **never**   | ❌ same decision, same ADR: an exhausted cascade returns the step to its owner with a message naming it, rather than putting work three live buttons could still finish beyond reach                                                                                                                                                                                    |
