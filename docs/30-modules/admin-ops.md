...
- **Both parties are notified of a dispute, and the thread still is not.** Amended 2026-09-09 by notifications slice 1: `dispute.raised` derives an inbox row for whichever party did not raise it, and `dispute.resolved` derives one for both, carrying which of the five resolutions it was. The room keeps its system message. **The node's working thread deliberately still gets nothing**, which is the half of the original decision that stands: a dispute is decided by an operator rather than negotiated between the parties, and a line in the thread would invite exactly that negotiation. An inbox row is addressed to one person and invites nothing. The row also omits `resolved_by`, so **an operator is never named to the parties** ([ADR-0028](../40-adr/0028-a-notification-is-derived-from-the-event.md)); `ops_actions` still records them for the people entitled to read it. Push, email and SMS remain unbuilt.
