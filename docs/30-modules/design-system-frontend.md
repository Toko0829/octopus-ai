...
**The artifact card shows its images, and fetches each link only when the card is on screen.** A creative step on a workspace with an image connector delivers a brief plus up to three JPEGs, and they render in the stream under the body. This replaces a count line that said "3 images, in the project panel.", whose stated reason (an inline image would mint a signed URL per artifact per re-render) was **wrong**: messages are keyed by id, so a broadcast re-renders without re-mounting, and an effect runs once per mount. The real cost, minting many ten-minute credentials at once when a room with many deliverables opens, is paid by `IntersectionObserver` instead: nothing is fetched until a card is visible, and the observer disconnects on first fire. `imageFilesOf` filters on the content type, so the first non-image file producer is a download rather than a broken picture, and a card written before the payload carried a project id falls back to the count line because there is no way to mint a link without one.
...
>
> **The question card is rendered now, and it is answered on the card.** `QuestionCard.tsx` renders an `action_embeds` row of component `question`. The questions stay in the message body in plain text, on the rule every card follows: the card is an enhancement of a readable message, never the only way to read it. What the card adds is one control per question. A budget band or a timeline is a **chip group** whose vocabulary comes from the contract (`BudgetBand`, `Timeline`), so the card and the route agree by construction; an audience, an offer or a target metric is a short field with its own Save. A task card renders one field per step, labelled with the step's title carried on the payload. Nothing asks the person to compose a paragraph, and nothing is submitted until they press the control for it.
>
> Three rules it holds. **A guess is marked as a guess**: a slot the model inferred is shown with its value and the word "my guess", and the control stays open so it can be corrected, because a wrong inference about somebody's business would otherwise be attributed to them. **State is a word, never a colour**: saved, my guess, recorded, and the closed banners ("Answered. I am planning with what you said.", "Closed: you started on something else.") are text, and the selected chip carries `aria-pressed` beside its wash. **The card patches itself from the action response**: Realtime broadcasts message inserts only, so `handleEmbedAction` in `ChatApp.tsx` now merges `payload` from the response when the route sent one. That fixed a second defect on the way: `CampaignCard` rendered "approved at 0" after approve, because the cap the route wrote into the payload never reached the card.
>
> The controls appear only for the owner and only while the card is pending; the server re-checks both. A card written before titles rode on it cannot label its fields and says so, pointing at the project panel, rather than offering a field with no name.
>
> **A plan that was replaced says so.** When the owner finishes the card before approving, the pending plan is moved to `expired` and the new one carries `supersedes`; `PlanCard` renders that state as "Replaced by the plan below, which uses what you answered." rather than in the rejected banner's words, and `mergeMessages` in `adapt.ts` marks the old card from the reference on the new one, because nothing broadcasts an embed state change and the old card would otherwise keep offering its buttons until a reload.
>
> **"About your business" on the project panel** (`AboutBusiness` in `ProjectPanel.tsx`) is the question card's rows inside the panel's hairline: what the workspace already knows, one field or chip group per fact, each saving on its own through `PATCH /api/rooms/:roomId/profile` so nothing typed here can erase a neighbour. Owner only, and absent rather than empty for anybody else. "Nothing yet" is shown for an empty fact, because a blank on a surface built for checking reads as something the agent forgot.
