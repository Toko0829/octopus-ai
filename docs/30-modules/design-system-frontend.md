...
**The artifact card counts its images and never shows one.** A creative step on a workspace with an image connector delivers a brief plus up to three JPEGs, and the card says "3 images, in the project panel." rather than rendering them. That is a security decision, not a layout one: the bucket is private, so an `<img>` means a signed URL, which is a ten-minute bearer credential, and the stream re-renders on every broadcast, so it would mint one per artifact per re-render for everybody with the room open. `imageFilesOf` filters on the content type, so the first non-image file producer is a download rather than a broken picture.
