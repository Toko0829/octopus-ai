...
- **A role is set by the server and never by its holder** (`20260831110000`). `authenticated` holds a column grant on `profiles` covering `display_name`, `jurisdiction` and `languages` only, and `private.guard_profile_role_self_service` refuses a `role` change from any writer carrying a JWT. `service_role` writes with no claims, so `auth.uid()` reads null and the server path is untouched — which is also what makes the guard fail in the useful direction: a future `SECURITY DEFINER` helper cannot launder a role change on a user's behalf, because the claims travel with the request rather than with the role.
  ...
  **`profiles.role` got its first writer in `20260902121000` and still authorises nothing.** `public.invite_node` promotes a user to `human_node`, running as `service_role` with no claims, which is exactly the bypass the guard was written with rather than an exemption added for it (`supabase/tests/rls_membership.sql:281-288` already pinned that path). Every authorisation in this system is still `rooms.owner_id` or an RLS policy on the caller's own row: no route, no policy and no UI branch reads `profiles.role`, including the `/node` surface, which routes on whether a `node_profiles` row exists because that is the fact and RLS enforces who can see it. The column is a backstop being filled in ahead of the readers that will need it, not a control that quietly became live.
