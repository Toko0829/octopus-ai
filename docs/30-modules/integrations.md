...
| Generation              | **Workspace connector** (Anthropic / OpenAI / Google) over a house-default OpenAI key | **the house default is live; `services/ai` speaks all four dialects and a key can be connected, checked and routed. Nothing sends a route yet.** Model ids live in the registry as data and a key is verified against the provider before it is stored ([ADR-0032](../40-adr/0032-reasoning-providers-are-workspace-connectors.md))                                                                               |
