...
| Generation              | **Workspace connector** (Anthropic / OpenAI / Google) over a house-default OpenAI key | **live end to end: a key is connected in the room rail, routed per role, and every plan, deliverable, campaign draft, replan and fallback answer runs on it and says which model wrote it.** Model ids live in the registry as data and a key is verified against the provider before it is stored ([ADR-0032](../40-adr/0032-reasoning-providers-are-workspace-connectors.md))                                                                                                          |
...
| **Creative generation** | **Google Gemini image** on the workspace's own key                                    | **live**: a visual step on a workspace with a `creative` route produces its brief and up to three JPEGs, generated by **Node** with the customer's key and stored as `asset` artifacts in the existing private bucket, so the bytes never depend on a provider-native tool ([ADR-0033](../40-adr/0033-the-first-byte-producer-is-the-workspace-image-connector.md)). The brief remains the deliverable and is written whether or not anything is drawn. Video and audio have no producer |
...
**Model keys are the second customer-held credential in this system, and the
credential half is now built.** `model_connections`, `apps/api/src/lib/envelope.ts`
and the four `/api/rooms/:roomId/models` routes landed together; nothing consumes
a route yet, so every agent run still uses the house key. [ADR-0032](../40-adr/0032-reasoning-providers-are-workspace-connectors.md)
