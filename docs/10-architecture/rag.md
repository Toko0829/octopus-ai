...
- **Grounded, cited, current, or refused, with one scoped exception ([ADR-0021](../40-adr/0021-a-labelled-ungrounded-tier.md)).** A **non-regulated marketing** question that the gate judges uncovered may be answered from general practice, labelled as uncited, if and only if retrieval returned chunks (so the question is in-domain) and the goal matches none of the regulated patterns in `ungrounded.is_regulated`. Such an answer is `grounded=False` with no citations and **cannot propose a plan**, so it cannot become a task DAG and cannot gate spending or publishing. Legal, tax and permit output is untouched: rule 10's citation mandate and rule 11's human escalation apply exactly as before.
  ...
  > **Which model writes that answer became a workspace choice** ([ADR-0032](../40-adr/0032-reasoning-providers-are-workspace-connectors.md)), and nothing in the paragraph above moves with it. The tier still returns `post_message` only, still carries `grounded=False` and no citations, and still declines regulated topics in `ungrounded.is_regulated` **before any provider is called**, which matters more with a customer's own key on the request rather than less. The label is still written by `ungrounded.frame` in code, which is the constraint that gets stronger with each additional vendor: a disposition asked of four models is four slightly different disclaimers. **Retrieval is untouched by connectors entirely.** Embedding and rerank stay in-process, the threshold is unchanged, and the gate's verdict logic is unchanged, because a connector is a generation decision. The reranker-off mode that would change any of that needs its own ADR. As of the first connector slice the seam exists in `services/ai` and nothing sends a target through it yet, so every answer here is still the house model until the Node wiring lands.
