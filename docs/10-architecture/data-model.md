...
**Live:** `projects`, `tasks`, `task_deps`, `task_runs` and the append-only `events` log (`20260813120000`), plus `artifacts` (`20260813160000`, described below). `agent_steps` and `tool_invocations` remain design-only. `playbook_versions` is deferred until a playbook compiler exists to write `compiled_dag` and `jurisdiction_pack`. `escalations` is **not a table**: the five facts it was to hold are the `task.routed` event and the next transition event on the same task ([ADR-0030](../40-adr/0030-an-escalation-is-an-event-and-a-playbook-is-the-card.md)), and the sketch above keeps the line for the reader who arrives from the Phase 0 spec.
...
| `ratings`            | ✅ live `20260908127000`                                                     | `(engagement_id, project_id, rater_id, ratee_id, direction, score 1..5, comment, created_at)`, unique on `(engagement_id, direction)`, append-only including for `service_role`. `direction` and `ratee_id` are derived by `submit_rating` from the engagement, so a caller cannot mislabel a score. Gated on `outcome = 'completed'`, which excludes `disputed_resolved` deliberately. Its writer is `trust_score`'s first, in the same transaction: `round(avg(score)/5, 4)`, NULL until the first rating and never 0                                                                                         |
| `disputes`           | ✅ live `20260908122000`                                                     | `(task_id, engagement_id, project_id, raised_by, raised_role, reason, evidence, from_state, resolution, release_amount, refund_amount, resolution_note, resolved_by, resolved_at)`. **No state column** (ADR-0016): `tasks.state` is the machine, and open is derived as `resolved_at is null` — a partial unique index on `(task_id) where resolved_at is null` makes "one open dispute per task" a constraint over that derivation. The four resolution columns are all-or-none, `engagements_outcome_iff_ended`'s shape. `from_state` is load-bearing: `rejection_upheld` is refused unless it is `rejected` |
| `ops_actions`        | ✅ live `20260908123000`                                                     | `(actor_id, action, subject_type, subject_id, reason, payload, created_at)`. `actor_id` and `reason` are both `not null` and that is the point: `events` cannot distinguish an operator from a sweep, because every `service_role` write lands there as `actor_kind = 'system'` with a null actor. RLS on, **no policy and no client grant**, and `update`/`delete`/`truncate` revoked from `service_role` too                                                                                                                                                                                                  |
