...
- **Model keys are encrypted at rest, and the decision that they must be is [ADR-0032](../40-adr/0032-reasoning-providers-are-workspace-connectors.md) rather than a later trigger.** A workspace connects its own Anthropic, OpenAI or Google key so its own models do the reasoning; that key is the second customer-held credential in this system and the first that arrives with encryption in the same change as its table. AES-256-GCM under a master key in `MODEL_KEY_SECRET`, with additional authenticated data bound to the row (`model_connections:{roomId}:{provider}:v{key_version}`), so a ciphertext copied into another row fails to open rather than decrypting under the wrong owner. `key_version` is on the row from the start; rotation tooling is not built and its trigger is the first rotation. **This is now built**, in one change as promised: `20260913120000` created the table, `apps/api/src/lib/envelope.ts` seals and opens, and `apps/api/src/routes/models.ts` is the only caller. The interval between a real key landing and the encryption landing was the exposure to avoid, and there was none. `envelope.test.ts` asserts the failures rather than the round trip: a tampered ciphertext, a tampered tag, a truncated tag, a swapped IV, the wrong master key and each of the three wrong AADs all throw, with one identical message so nothing tells an attacker which half of a guess was right.\n\n > **Accepted risk, recorded.** What remains is a JOINT leak: the database **and** `MODEL_KEY_SECRET` together. Neither alone yields a key, which is strictly better than the channel tokens below, and it is not nothing. **Trigger to narrow it: the first production deployment**, which should hold the master key in a KMS rather than an environment variable, and should terminate TLS between `apps/api` and `services/ai` rather than relying on a private compose network for the one hop where the key travels decrypted.

  > **Supabase Vault was rejected, and the reason is specific to this system.** Vault decrypts inside Postgres for any role that can read `vault.decrypted_secrets`, and `services/ai` holds `service_role`. Putting model keys there would let the Python container read every customer's key with a select, which defeats the one property the encryption exists to buy: **decryption confined to the Node code that builds the outbound request.** The master key is provisioned to the API app alone, and the compose file now sets `MODEL_KEY_SECRET: ''` on the AI service so it cannot inherit one through `env_file`, since that container takes `apps/api/.env` wholesale and `environment` beats `env_file`.
  >
  > The table carries **no policy and no grant**, like `channel_connections`, so the same reasoning applies: RLS filters rows and not columns, and a select policy that returned the row would return the key with it. What a member may see is an API projection carrying a key hint and never the key, and the contract type has no field for one.
  >
  > **A missing master key fails the run rather than falling back.** A workspace with routes configured and no `MODEL_KEY_SECRET` gets a notice naming the variable. A silent fallback to the house key would send a customer's work to a provider they did not choose and bill it to us, which is a failure that looks like success from every surface.

- **A customer's generated images live in the private artifacts bucket, and the reasoning about them is entirely inherited** ([ADR-0033](../40-adr/0033-the-first-byte-producer-is-the-workspace-image-connector.md)). A creative step on a workspace with an image connector writes JPEGs to `<project_id>/<artifact_id>/` in the same bucket a human node's proof already goes to, so the same `storage.objects` select policy decides who can see them and the same signed-URL route is the only way to read one. Nothing new was granted, and the tenancy argument in [data-model.md](data-model.md) covers them unchanged.

  > **The bytes never touch `services/ai`.** The core emits a proposal describing what to draw; `apps/api` calls the vendor with the workspace's key and uploads the result. That keeps [ADR-0006](../40-adr/0006-python-ai-service-node-backend.md) true for the first non-text producer and keeps the customer's credential inside the one process that already holds it. The prompt it carries is derived from a model's own brief and is treated as untrusted data (rule 8): bounded on both sides of the seam, sent as a JSON field, and never interpolated into a URL or a header.
  >
  > **A card never carries a link to one.** `ArtifactEmbedPayload` holds file ids and content types. A signed URL is a ten-minute bearer credential and an embed row is stored and re-broadcast on every room update, so a link there would be a credential written to a table and handed to everyone in the room for as long as the row lives. The panel mints one per click, for the person who clicked.
  >
  > **`artifacts.content_type` is not a safety control**, and the column comment says so. What decides how a browser treats the bytes is the type Storage was given at upload, and what stops a hostile one being served from our own origin is that the bucket is private and every read is a short-lived signed URL on Storage's own host.
...
- **Where a workspace's text goes is now partly the customer's choice, and partly not** ([ADR-0032](../40-adr/0032-reasoning-providers-are-workspace-connectors.md) decision 5). On a workspace that has connected its own model key, plan, replan, execute, campaign-draft and ungrounded-fallback prose reaches the provider **that customer chose, under their own account and their own terms with that vendor**. Three calls do not follow it: query decomposition, the groundedness gate and intake stay on the house model, because they are pinned at `temperature: 0` and their thresholds were measured there, and routing a safety threshold by customer configuration is not a privacy improvement. **The cost of that is stated rather than left to be discovered:** the goal sentence, the intake answers and the retrieved sources block, which can include the room's own documents, still reach the house provider on a BYOK workspace. **Trigger to revisit: the first customer whose data-processing agreement forbids it**, whose only two answers are a per-workspace classification route with a threshold recalibration, or declining the workspace. Retrieval itself is unaffected in both cases, because embedding and rerank run in-process and no corpus text leaves it ([ADR-0008](../40-adr/0008-local-bge-m3-embeddings.md), [ADR-0009](../40-adr/0009-local-reranker.md)).
