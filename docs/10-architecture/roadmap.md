...
>
> **Closed since this was written:** pgTAP RLS coverage now exists (`supabase/tests/rls_membership.sql`, 26 assertions, including that an expired node sees nothing at all), so rule 18's zero-coverage debt is paid for room-scoped membership. Thread-scoped membership is covered now (`thread_scope.sql`, 55 assertions) and so are the marketplace's first writers (`node_onboarding.sql`, 46). Ops/admin paths remain uncovered, and have no code to cover. **All three counts were stale here** (22, 46 and 36) because each suite grew after this paragraph was written and nothing pointed back at it; they are now read from each suite's own `plan(N)`, which is where a module doc's **Current shape** section takes them from too.
>
...
| `playbook_versions` (a compiled archetype × jurisdiction-pack DAG with its pack version)                                                          | the playbook compiler: today a plan is the planner's output and its record is the approved card ([ADR-0030](../40-adr/0030-an-escalation-is-an-event-and-a-playbook-is-the-card.md))                                                                                                     |
