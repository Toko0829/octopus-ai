...
>
> **A provider's output is a lead, never a source.** What it is allowed to do is point at a gap: `retrieval_gaps` records the question a refusal or a labelled ungrounded answer came from and, **since `20260913123000`, the provider and model that answered it**; `feedback_events` records a person's verdict; the crawl registry takes candidate sources a human reviews in a diff. Every one of those is a human deciding what enters the corpus. Nothing auto-ingests.
>
> **The attribution columns are the sharpest test of that rule, because they are the ones that would be most useful to break it with.** A queue that names the model which answered each gap is exactly the shape a distillation set has, and it is not one: nothing reads those rows into the corpus, no answer recorded there is trained on, and what the columns buy is a person reading a queue with one more fact in it. They exist so that two rows with the same core, the same gate reason and the same near misses stop averaging into one number once a workspace routes Fallback to its own connector.
...
| ------------------ | --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| User in chat       | approve / reject / edit, thumbs, comments     | `feedback_events` **(live, both subjects)**. Helpful / not helpful on a model-written message with no card landed with `20260913124000`, and is the first label in this system that attaches to a **model** rather than to a plan. The control itself is slice 5; the route and the table are here |
...
The second query **ran live against real rows** the day the route landed, returning a model with one `helpful` and one `not_helpful` against it, which is also the demonstration that a changed mind appends rather than overwrites: the table has no client `UPDATE`, so a second verdict is a second row and the latest one wins.
...
**What a label may be attached to is narrower than what a person might click on**, and the three refusals are worth stating because each protects the rate from a different kind of nonsense. A person's or a node's message is not our output. A run notice, a sweep notice, a waiting digest and a recorded answer are Octopus's own copy written in TypeScript, so `messages.model` is null on them and a thumbs-down would be a label on a sentence no model composed. And a message carrying a card already has a verdict with consequences attached, so rating it as prose would count one output twice. The route answers 409 to all three.
