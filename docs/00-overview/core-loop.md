...
3. **Durable run starts.** Fastify recognizes a new growth goal (or an explicit `POST /projects`), creates a `project` row, and triggers `tasks.trigger('agent.run', { projectId, roomId })`. **Returns `202 + runId`.** No long work in the request thread.
   ...
   **3a. Intake, before anything is retrieved.** A vague one-liner is clarified first: `POST /intake` turns it into the playbook's slots (ICP, offer, target metric, budget band, timeline) and scores two things in code, how much of what we need is known and how close the request sits to what the corpus covers.
   ...
   Four ways a turn can end. A request naming a specific problem passes straight through and is asked nothing. A whole-funnel request gets **one batch of questions, at most twice**, because [vision.md](vision.md) counts user touches as a guardrail to drive down. A greeting is answered with the opening question rather than a refusal, since it is not a request we cannot serve, it is not a request. And a genuine request from another field is **declined and then redirected once**: someone opening a cafe cannot be helped to open it and may well want customers through its door. That redirect is bounded at two attempts, so following a thread never becomes refusing to take no for an answer.
