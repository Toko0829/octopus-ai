...
- **Services** — `apps/web` (Next.js BFF), `apps/api` (Fastify), `apps/matcher`, `apps/agent` (durable tasks, not built; the ticker runs inside `apps/api` today), Supabase (Postgres/RLS/Realtime/Storage, and durability via [ADR-0010](../40-adr/0010-postgres-durable-runner.md)), channel integrations (ad platforms, social, analytics, creative-gen), Stripe Connect (escrow/payouts).
...
3. **Durable run starts.** Fastify recognizes a new growth goal (or an explicit `POST /projects`), creates a `project` row, and triggers `tasks.trigger('agent.run', { projectId, roomId })`. **Returns `202 + runId`.** No long work in the request thread.
   ...
   **3a. Intake, before anything is retrieved.** A vague one-liner is clarified first: `POST /intake` turns it into the playbook's slots (ICP, offer, target metric, budget band, timeline) and scores two things in code, how much of what we need is known and how close the request sits to what the corpus covers.

   Four ways a turn can end. A request naming a specific problem passes straight through and is asked nothing. A whole-funnel request gets **one card of at most four questions, at most twice**, because [vision.md](vision.md) counts user touches as a guardrail to drive down, and **the plan is written in the same run**: the room is told work has started, a plan with visibly empty stages arrives a minute or two later, and the card sits beside it as what would sharpen it ([ADR-0029](../40-adr/0029-a-question-is-answered-on-the-card.md)). The questions are **answered on the card**, one chip or one short field at a time, never by typing a paragraph into the composer: each answer is saved as it lands, the card closes itself when what it needs is there or when the owner says to plan with what they have said, and a sharper plan replaces the pending one, or a plan-change card arrives if the first was already approved. **The workspace remembers the answers**: audience, offer, budget band and timeline are facts about the business, stored per room and handed to intake before the first question, so the second goal in a room is asked nothing. A greeting is answered with the opening question rather than a refusal, since it is not a request we cannot serve, it is not a request. And a genuine request from another field is **declined and then redirected**: someone opening a cafe cannot be helped to open it and may well want customers through its door. Neither reply opens a card, because **every chat message is a goal**: nothing a person types is read as the answer to something else.

   Intake **does not use RAG**, deliberately: what someone sells is not in the corpus. It hands step 4 a sharper query, and it grants nothing, so the groundedness gate in step 4 still runs unchanged. Details in [ai-orchestrator.md](../30-modules/ai-orchestrator.md).
...
7. **Approval / connect / escalate.** Actions with real-world impact gate: **connecting the user's ad/social accounts**, **spending ad budget**, **publishing** public content, or a task needing **human judgment/taste/relationships** (brand direction, high-end video edit, influencer outreach, account verification). The agent posts an **approval embed** to the user, or the router parks a human-owned step at `escalated` for the owner to send to the marketplace; on a human task, **escrow is held**, the matcher finds an eligible expert node, and the step **waits in its row** at zero compute ([ADR-0010](../40-adr/0010-postgres-durable-runner.md)).
