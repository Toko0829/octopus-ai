...
- **Status:** Phase 1, exit gate **substantially met**. Phase 0 is complete (source-of-truth documentation set + Turborepo monorepo scaffold).
  ...
  **Retrieval is measured, not asserted.** Hybrid search (dense + sparse fused by RRF in SQL) then a Cohere cross-encoder, with a threshold that drops weak chunks; **query decomposition** splits a broad goal into per-stage sub-queries. A golden-set **eval gate runs in CI** (positive recall 1.00, coverage 1.00, zero negative leaks). When nothing clears the threshold the agent **refuses and says why** rather than inventing a plan, and decomposition is additive to grounding rather than a source of it. Embeddings can run locally on **BAAI/bge-m3** ([ADR-0008](docs/40-adr/0008-local-bge-m3-embeddings.md)) or on OpenAI. RLS is covered by a **pgTAP suite** (22 assertions), including that an expired node sees nothing at all.

  **Not built, and not claimed:** crawlers and the freshness pipeline, the **generation** eval (Ragas/DeepEval faithfulness needs an LLM judge, so retrieval is gated and generation is not), durable runs (Trigger.dev, blocked on credentials), OpenAPI generation, a Node-side test harness, and a formal anti-slop design review. The corpus is ten internally-authored documents, so nothing is externally cited yet. **A production Cohere key is now a prerequisite**, not a nicety: decomposition costs one rerank per sub-query and the trial key allows ten calls a minute. That is no longer a prediction, it took CI down twice. A client-side ceiling (`COHERE_RERANK_RPM`) makes the trial key survivable, at the price of a ten-minute eval run and a production ceiling of roughly one user goal per minute.
