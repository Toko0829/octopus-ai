...
- **Status:** Phase 1, in progress. Phase 0 is complete (source-of-truth documentation set + Turborepo monorepo scaffold). Landed since: the Discord-style chat shell (`apps/web`, mock-driven), the chat schema on Supabase (`rooms` / `channels` / `room_members` / `messages` with membership RLS), and the **server-authoritative chat write path** in `apps/api` — post + since-cursor catch-up, with Realtime broadcast-from-Postgres verified end-to-end. Still to wire: `apps/web` calling those endpoints instead of mocks, and OpenAPI generation. See [CHANGELOG.md](CHANGELOG.md) for what exists and [roadmap.md](docs/10-architecture/roadmap.md) for what is next.
