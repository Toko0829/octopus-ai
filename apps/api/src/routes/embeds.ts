 ...
 *
 * Four checks, in order, none of which the client can supply an answer for:
 *
 ...
 *   2. **`required_role`**, re-checked here. The UI is told the role so it can
 *      disable what the caller cannot do, but that is a courtesy. A rule enforced
 *      only in React is not enforced.
 *   3. **State**, so an embed is single-use. Approving twice is two approvals,
 ...
 *   4. **A conditional update**, so two concurrent approvals cannot both win.
...
export async function embedRoutes(app: FastifyInstance, opts: EmbedRoutesOptions): Promise<void> {
  ...
  app.post(
    ...
    async (request, reply): Promise<FastifyReply> => {
      ...
      // (2) required_role. Only 'owner' exists today; an unknown value denies
      ...
      // (4) Conditional on still being pending, so a double submit updates once.
      ...
      request.log.info({ embedId, roomId, userId, action }, 'embed action recorded');
      return reply.code(200).send({ id: updated.id, state: updated.state });
    },
  );
}
